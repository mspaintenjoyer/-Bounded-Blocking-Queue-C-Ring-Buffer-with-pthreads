...
struct bqueue {
    ...
    unsigned char* buf; // cap * item_size

    pthread_mutex_t mu;
    pthread_cond_t  not_empty;
    pthread_cond_t  not_full;
    int closed; // optional extension (unused here)
};

static size_t idx_step(size_t i, size_t cap) { return (i + 1) % cap; }
...
int bq_pop(bqueue_t* q, void* out) {
    ...
    unsigned char* src = q->buf + (q->head * q->item_size);
    memcpy(out, src, q->item_size);
    q->head = idx_step(q->head, q->cap);
    q->len--;
